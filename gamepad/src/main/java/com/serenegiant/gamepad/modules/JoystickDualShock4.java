 ...
 *
 * File name: JoystickDualShock4.java
 *
...
public class JoystickDualShock4 extends JoystickParser {
...
//	private static final String TAG = JoystickGeneral.class.getSimpleName();
	...
	public JoystickDualShock4(final InputDevice device) {
		super(device);
	}
	...
	protected void update() {
		...
		// 右アナログスティックX
		if (mAxes.indexOfKey(MotionEvent.AXIS_Z) >= 0) {
		...
		if (mAxes.indexOfKey(MotionEvent.AXIS_RX) >= 0) {
			keyCount[KEY_LEFT_2] = (int)(mAxisValues[mAxes.indexOfKey(MotionEvent.AXIS_RX)] * 127 + 127);
			if ((keyCount[KEY_LEFT_2] == 0) && (mKeys.indexOfKey(KeyEvent.KEYCODE_BUTTON_L1) >= 0)) {
		...
		// 上端キーR2
		if (mAxes.indexOfKey(MotionEvent.AXIS_RY) >= 0) {
			keyCount[KEY_RIGHT_2] = (int)(mAxisValues[mAxes.indexOfKey(MotionEvent.AXIS_RY)] * 127 + 127);
			if ((keyCount[KEY_RIGHT_2] == 0) && (mKeys.indexOfKey(KeyEvent.KEYCODE_BUTTON_R1) >= 0)) {
		...
		if (mKeys.indexOfKey(KeyEvent.KEYCODE_BUTTON_X) >= 0) {
			keyCount[KEY_RIGHT_UP] = mKeys.get(KeyEvent.KEYCODE_BUTTON_X);
		} else {
		...
		if (mKeys.indexOfKey(KeyEvent.KEYCODE_BUTTON_C) >= 0) {
			keyCount[KEY_RIGHT_RIGHT] = mKeys.get(KeyEvent.KEYCODE_BUTTON_C);
		} else {
		...
		if (mKeys.indexOfKey(KeyEvent.KEYCODE_BUTTON_B) >= 0) {
			keyCount[KEY_RIGHT_DOWN] = mKeys.get(KeyEvent.KEYCODE_BUTTON_B);
		} else {
